...
public class ChanceNode extends TreeNode {
	...
	public ChanceNode(TreeNode parent, BuildNode node) {
		...
		// Chance nodes advance on the turn of the player by one - the next
	...
	public TreeNode selectNextNode() {
		...
		if (children.get(roll) == null) {
			...
			if (roll != 7) {
				...
				handleResourceDiscard(gameCopy);
			}
	...
	private void handleResourceDiscard(ReducedGame game) {
		List<ReducedPlayer> players = game.getPlayers();
		for (ReducedPlayer player : players) {
			if (player.getNumResources() > 7) {
				{
					int[] discarded = MCTSUtils.getDiscard(player.getResources(), player.getNumResources()/2);
					game.handlePlayerDiscard(discarded, player.getPlayerNumber());
				}
			}
